...
function enumerateStandardSolutions(looseTiles, revealedGroups) {
  ...
  return looseSolutions.map((groups) => ({
    shape: 'standard',
    groups: [...revealedGroups, ...groups],
  }));
}
...
export function findHuSolutions(looseTiles, revealedGroups = []) {
  ...
  const knownGroups = revealedGroups.every((group) => GROUPS[group.kind]);
  if (!knownGroups) return [];
...
export function detectPatterns(solution) {
  ...
  const numberedSuits = new Set(
    tiles.filter((tile) => tile.suit !== 'honor').map((tile) => tile.suit),
  );
  ...
  return patterns.length ? patterns : ['普通胡'];
}
...
function scoreEffect(effect, groups, actionsRemaining) {
  if (!effect) return 0;
  if (effect.kind === 'group') {
    return countGroups(groups, (group) => group.kind === effect.groupKind) * effect.value;
  }
  if (effect.kind === 'meldKinds') {
    return countGroups(groups, (group) => effect.groupKinds.includes(group.kind)) * effect.value;
  }
  if (effect.kind === 'honorMeld') {
    return countGroups(
      groups,
      (group) => group.kind !== 'pair' && group.tiles[0].suit === 'honor',
    ) * effect.value;
  }
  if (effect.kind === 'remainingActions') return actionsRemaining * effect.value;
  return 0;
}
...
export function scoreSolution(solution, {
  ...
  const groupChips = solution.groups.reduce(
    (sum, group) => sum + SCORING_CONFIG.groupBase[group.kind],
    0,
  );
  const bonusEvents = [];
  let bonusChips = 0;
  ...
  for (const charmId of charmIds) {
    ...
    if (!charm) continue;
    const value = scoreEffect(charm.effect, solution.groups, actionsRemaining);
    bonusChips += value;
    bonusEvents.push({ source: 'charm', id: charm.id, name: charm.name, value });
  }

  const general = generalId ? FORTUNE_GENERALS[generalId] : null;
  if (general) {
    const value = scoreEffect(general.effect, solution.groups, actionsRemaining);
    bonusChips += value;
    bonusEvents.push({ source: 'general', id: general.id, name: general.name, value });
  }

  const patterns = detectPatterns(solution);
  const patternBonus = patterns.reduce(
    (sum, pattern) => sum + (SCORING_CONFIG.patternBonus[pattern] ?? 0),
    0,
  );
  const multiplier = 1 + patternBonus;
  const baseChips = SCORING_CONFIG.huBase + groupChips + bonusChips;
  const total = Math.floor(baseChips * multiplier);

  return {
    ...solution,
    patterns,
    huBase: SCORING_CONFIG.huBase,
    groupChips,
    bonusChips,
    baseChips,
    patternBonus,
...
export function findBestHu(looseTiles, revealedGroups = [], scoringOptions = {}) {
  const scored = findHuSolutions(looseTiles, revealedGroups)
    .map((solution) => scoreSolution(solution, scoringOptions));
  scored.sort((left, right) => right.total - left.total
    || right.patterns.length - left.patterns.length
    || Number(right.shape === 'sevenPairs') - Number(left.shape === 'sevenPairs')
