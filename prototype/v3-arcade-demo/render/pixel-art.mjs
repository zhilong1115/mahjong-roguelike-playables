/* =========================================================
   像素绘制：低分辨率 canvas + alpha 硬化 → 放大后仍然锐利
   全部离线自绘，不依赖外部字体或图片。
   ========================================================= */
...
function bird(context, cx, cy) {
  ...
  const originX = cx - (width >> 1);
  const originY = cy - (height >> 1);
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      const code = BIRD[y][x];
      if (code && code !== '.') px(context, originX + x, originY + y, 1, 1, palette[code]);
    }
  }
}
...
function backSource() {
  ...
  canvas.height = TILE_H;
  const context = canvas.getContext('2d');
  px(context, 0, 0, TILE_W, TILE_H, '#1d3f6e');
  px(context, 1, 1, TILE_W - 2, TILE_H - 2, '#2f5f9e');
  px(context, 2, 2, TILE_W - 4, TILE_H - 4, '#25508a');
  for (let y = 4; y < TILE_H - 4; y += 4) {
    for (let x = 4; x < TILE_W - 4; x += 4) px(context, x, y, 2, 2, '#3b76bd');
  }
  px(context, 1, TILE_H - 3, TILE_W - 2, 2, '#132b4d');
  cache.set(key, canvas);
  return canvas;
}
...
export function createTileCanvas(tile, scale = 2, { dim = false, back = false } = {}) {
  return scaled(back ? backSource() : tileSource(tile, dim), scale, 'pxc tileFace');
}
...
export function setPixelText(node, text, size, color, shadow = null, maxScale = 3) {
  ...
  node.innerHTML = '';
  const image = pixelText(text, size, color, shadow, available > 8 ? available : 0, maxScale);
...
export function createSealCanvas(character, { paper = '#e8d4a2', ink = '#8c2f27', edge = '#2a1c12', scale = 2 } = {}) {
  const key = `seal|${character}|${paper}|${ink}|${edge}`;
  ...
  if (!source) {
    ...
    px(context, 0, 0, width, height, edge);
    px(context, 1, 1, width - 2, height - 2, paper);
    px(context, 3, 3, width - 6, 1, ink);
    px(context, 3, height - 4, width - 6, 1, ink);
    px(context, 3, 3, 1, height - 6, ink);
    px(context, width - 4, 3, 1, height - 6, ink);
    glyph(context, character, width / 2, height / 2, 19, ink);
