...
core/       规则内核，纯 JS，不碰 DOM
  ...
  run       一局状态机：东南两圈 × 三关 × 一副、跳局、五家百宝阁、西圈加赛
content/    五系统一 Library，以及圈关 / 圈主 / 牌组 / 手气 / 牌帖数据
...
- 只用具名导出，不用默认导出。
- 五系功能卡只在 `content/library.mjs` 增删改；下架先设 `enabled: false`，不直接删掉旧存档可能引用的 id。
- 新玩法效果类型不够用时，同时扩展 Library 校验与 `core/effects.mjs` / 对应事件处理器。
...
`Proposed` 的 `docs/decisions/0017-short-run-shop-cadence.md`；当前进度见 `docs/09-progress.md`。
