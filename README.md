...
- 当前 `Proposed` 外层切片 = 东 / 南 / 西三圈 × 闲局 / 庄局 / 圈主三关 × 每关两副；闲局和庄局可跳过换手气，圈主必须打
- 每个打赢的非最终关后进入百宝阁，因此全打一局最多经过 8 家商店；跳局不进商店
- 每副 14 张结构牌、5 次换牌、6 个开运位；亮组花一格求签，通常三选一，广缘兆应验时四选一
- 灵签已显示银 / 金 / 彩签阶；鸿运与广缘可在单格待缘位保留到下一次求签后消费
- 得分 `牌值 × 番势`，结算按固定顺序逐条播放动画，可点击快进
- 五系构筑：**灵签**（本副）· **番谱**（升番种）· **福将**（占将位）· **牌骨**（牌种静态材质）· **牌印**（牌种事件触发）
- 开始界面可选牌组；存档当前为 schema v4，并保留 v1 → v4 逐版本迁移；随机全部走可复盘 seed
...
正式版框架与 `0013` 圈关切片已经落在 `src/`，但圈关结构仍是 `Proposed`，不是冻结规则。接下来按顺序做：真人试玩整局时长与跳局选择 → 重新标定九关目标和最多八家商店的经济 → 单文件打包与体积预算 → 平台 SDK 接入。详见 [`docs/09-progress.md`](docs/09-progress.md)。
